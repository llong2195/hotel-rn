...
class TimePicker extends PureComponent<TimePickerProps, States> {
  ...
  showTimePicker = () => {
    ...
    let select = this.reverse
      ? [title1, current1, title2, current2, title3, current3]
      : [current1, title1, current2, title2, current3, title3];

    if (current3 == undefined && current2 != undefined) {
      if (this.reverse) {
        select = ['', title1, current1, title2, current2, ''];
      } else {
        select = ['', current1, title1, current2, title2, ''];
      }
    }
