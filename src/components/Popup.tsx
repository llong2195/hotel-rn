...
class Popup extends PureComponent<Props, States> {
  ...
  show = (props: PropsPopup) => {
    this.dataPopup = {
      ...
      height: props.height
        ? Math.min(R.DEVICE_HEIGHT, props.height)
        : R.DEVICE_HEIGHT,
      data: props.data || [],
    };
    this.open();
  ...
  handleScroll = (event: any) => {
    if (
      Math.round(event?.nativeEvent?.contentOffset?.x / R.DEVICE_WIDTH) !==
      this.state.page
    ) {
      this.setState({
          ...
          {this.dataPopup?.data.map((item: any, index: number) => (
            <View
              style={{
                ...
                backgroundColor: page === index ? colors.mainColor : '#B5B5B5',
