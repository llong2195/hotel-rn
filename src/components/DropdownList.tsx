          ...
          >
            {filter ? (
              <InputBase
                style={{
                  alignSelf: 'center',
                  width: R.DEVICE_WIDTH - verticalScale(30),
                  marginTop: verticalScale(10),
                ...
                onChangeText={txt => {
                  const newDataFillter = data.filter(item => {
                    const itemData = removeVietnameseTones(
                      item.name,
                    ).toUpperCase();
            ...
            <ScrollView
              style={{flex: 1, width: '100%', marginBottom: verticalScale(10)}}
              showsVerticalScrollIndicator={false}
            >
              {newData?.map((item, i) => renderItem(item, i))}
            </ScrollView>
            ...
            <View style={[{flexDirection: 'row'}, viewButton]}>
              {titleButton1 ? (
                <BaseButton
                  title={titleButton1}
                  disable={buttonDisable1}
                  style={styleButton1}
                  onPress={_onButton1Pressed}
                />
              ) : null}
              {/* {console.log(data)} */}
              {titleButton2 ? (
                <BaseButton
                  title={titleButton2}
                  disable={buttonDisable2}
                  style={styleButton2}
                  onPress={_onButton2Pressed}
                />
              ) : null}
            </View>
          </View>
